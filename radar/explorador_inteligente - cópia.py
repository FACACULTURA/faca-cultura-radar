...
PALAVRAS_CAMINHO = [
    ...
    "comunicados", "imprensa", "institucional"
]
...
def score_fonte(fonte, url):
    ...
    if any(p in texto for p in ["festival", "mostra", "film commission", "instituto"]):
        score += 20
...
def score_link(texto, url, pagina_relevante=False):
    ...
    if any(lixo in texto_total for lixo in [
        ...
        "linkedin", "whatsapp", "privacy", "cookies"
    ]):
        score -= 20
...
def explorar_fonte(
    ...
    while fila and len(visitadas) < limite_paginas:
        ...
        for fonte in fontes:
    try:
        peso = fonte.get("peso_exploracao", 1)
        ...
        print(f"[ERRO NA FONTE] {fonte.get('nome')}: {e}")
    ...
    fontes_teste = [
        {
            ...
            "tipo": "fonte_mae",
            "area": ["cultura", "audiovisual"],
        }
    ]

    executar_exploracao(fontes_teste)