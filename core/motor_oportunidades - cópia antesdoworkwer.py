...
def parse_data(item):
    prazo = item.get("prazo") or item.get("deadline") or item.get("data_publicacao")
...
def ordenar_feed(dados):
    ...
    def prioridade(item):
        ...
        return (
            grupo,
            data,
            (item.get("titulo_principal") or item.get("titulo") or "").lower()
        )
...
def etapa_validacao():
    ...
    csv_path = BASE_DIR / "output" / "tudo.csv"
    ...
    for idx, item in enumerate(dados, start=1):
        ...
        try:
            ...
            if leitura.get("titulo_semantico"):
                item_semantico["titulo_principal"] = leitura.get("titulo_semantico")
            ...
            if categoria in ["editais", "oportunidades", "noticias", "licitacoes"]:
                feed.append(item_semantico)
            elif categoria in ["revisao", "noticia_pista"]:
        ...
        if idx % 25 == 0:
            print(f"… {idx}/{total} | feed={len(feed)} revisao={len(revisao)} descartados={len(descartados)}")
    ...
    for item in feed:
        ...
        item["relevancia_estrategica"] = contexto["relevancia_estrategica"]
    ...
    try:
        ...
        print(f"⚠️ Peneira falhou, mantendo feed bruto: {e}")
    ...
    salvar_json(FEED_FINAL, saida_final)
    ...
    print(f"📁 Feed salvo em: {FEED_FINAL}")
