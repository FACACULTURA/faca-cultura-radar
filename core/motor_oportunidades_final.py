...
from processamento.explorador_inteligente_v2 import enriquecer_com_exploracao
from processamento.extrator_semantico_v2 import extrair_semantica_v2
from processamento.normalizador_feed import normalizar_item_feed
...
def etapa_validacao():
    ...
    for idx, item in enumerate(dados, start=1):
        try:
            ...
            item_explorado = enriquecer_com_exploracao(item_enriquecido)
            item_semantico = extrair_semantica_v2(item_explorado)
            item_normalizado = normalizar_item_feed(item_semantico)

            if item_normalizado:
                feed.append(item_normalizado)
            else:
                texto = json.dumps(item_semantico, ensure_ascii=False).lower()
        ...
        if idx % 25 == 0:
            print(
                ...
                f"feed={len(feed)} revisão={len(revisao)} descartados={len(descartados)}"
            )
